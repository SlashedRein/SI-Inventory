    ...
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-center mb-4 gap-3">
        ...
        <div class="d-flex gap-2">
            ...
            <a href="{{ route('penjualan.create') }}" class="btn btn-primary d-flex align-items-center gap-2 rounded-3 shadow-sm px-4">
                ...
                <span class="d-none d-md-inline">Kasir Baru</span>
            </a>
        </div>
    </div>
                ...
                <tr class="border-bottom-0">
                    ...
                    <td class="text-end pe-3">
                        <div class="d-flex justify-content-end gap-2">
                            <button class="btn btn-sm btn-light text-info border rounded-2" 
                                    onclick="showDetail({{ $item->id_penjualan }}, '{{ $item->customer->nama_customer ?? 'Umum' }}', {{ $item->detail }})" 
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow-lg rounded-4">
            ...
            <div class="modal-footer border-0">
                ...
                <button type="button" class="btn btn-primary rounded-3 px-4" onclick="window.print()"><i class="bi bi-printer me-2"></i> Cetak</button>
            </div>
        </div>
    </div>
</div>
    ...
    function showDetail(id, customer, details) {
        ...
        if(details.length > 0) {
            details.forEach(item => {
                let namaProduk = item.produk ? item.produk.nama_produk : 'Produk Dihapus';
