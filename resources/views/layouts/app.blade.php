...
<head>
    ...
    <style>
        :root {
            ...
            --primary: #E07A5F;       /* Terracotta (Action) */
            ...
            --sidebar-width: 260px;
            --sidebar-collapsed-width: 80px; /* Ukuran saat sidebar nutup */
            --header-height: 70px;
            --radius-card: 16px;
        }
        ...
        .nav-link:hover, .nav-link.active {
            background-color: var(--primary); color: #fff; transform: translateX(3px);
            box-shadow: 0 4px 15px rgba(224, 122, 95, 0.4);
        }
        .nav-link i { font-size: 1.1rem; transition: 0.2s; }
        ...
        .btn-primary { background-color: var(--primary); border: none; padding: 10px 20px; border-radius: 10px; }
...
<body>
    ...
    <script>
        // Script Toggle Sidebar (Desktop & Mobile)
        const btnToggle = document.getElementById('btnToggle');
        btnToggle?.addEventListener('click', function() {
            if (window.innerWidth >= 992) {
                document.body.classList.toggle('sidebar-collapsed'); // Desktop: Kecilin sidebar
            } else {
                document.body.classList.toggle('sidebar-open'); // Mobile: Munculin sidebar
            }
        });
    </script>
    @stack('scripts')
